...
async function GAMELOGIC(delta){
	...
	if (test.position.x < -(Engine.screenSize.x / 2 - 25)){
		moveDir.x = 1;
	}
	if (test.position.x > (Engine.screenSize.x / 2)){
		moveDir.x = -1;
	}
	if (test.position.y < -(Engine.screenSize.y / 2 - 25)){
		moveDir.y = 1;
	}
	if (test.position.y > (Engine.screenSize.y / 2)){
		moveDir.y = -1;
	}
